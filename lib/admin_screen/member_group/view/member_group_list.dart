...
class _MemberGroupListState extends State<MemberGroupList> with NameFetchingMixin {
  ...
  // Implement the required getters for the mixin
  @override
              ...
              return Card(
                ...
                child: ListTile(
                  title: FutureBuilder<String>(
                    future: getGroupName(groupId), // Using mixin method
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      FutureBuilder<String>(
                        future: getUserName(userId), // Using mixin method
                        builder: (context, snapshot) {
                          if (snapshot.connectionState == ConnectionState.waiting) {
                            return const Text('Loading user...');
                          }
                          return Text('Member: ${snapshot.data ?? 'Unknown User'}');
                        },
                      ),
                  ...
                  trailing: PopupMenuButton<String>(
                    icon: Container(
                      ...
                      decoration: BoxDecoration(
                        color: MemberGroupHelpers.getStatusColor(group['status']),
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            MemberGroupHelpers.getStatusText(group['status']),
                            style: TextStyle(
                              color: Colors.white,
                    ...
                    itemBuilder: (BuildContext context) => [
                      PopupMenuItem<String>(
                        ...
                        child: Row(
                          children: [
                            Icon(MemberGroupHelpers.getStatusIcon(MemberGroup.pending), 
                                 color: MemberGroupHelpers.getStatusColor(MemberGroup.pending), 
                                 size: 16),
                      ...
                      PopupMenuItem<String>(
                        ...
                        child: Row(
                          children: [
                            Icon(MemberGroupHelpers.getStatusIcon(MemberGroup.active), 
                                 color: MemberGroupHelpers.getStatusColor(MemberGroup.active), 
                                 size: 16),
                            const SizedBox(width: 8),
                            const Text('Active'),
                          ],
                      ...
                      PopupMenuItem<String>(
                        value: MemberGroup.rejected.toString(),
                        child: Row(
                          children: [
                            Icon(MemberGroupHelpers.getStatusIcon(MemberGroup.rejected), 
                                 color: MemberGroupHelpers.getStatusColor(MemberGroup.rejected), 
                                 size: 16),
                            const SizedBox(width: 8),
                            const Text('Rejected'),
                          ],
